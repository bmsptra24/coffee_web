...
<div class="bg-white p-8 rounded-2xl border-2 border-gray-200 max-w-2xl mx-auto">
    <form action="<?= site_url('admin/promos/update/' . $promo['id']) ?>" method="post" enctype="multipart/form-data">
        <div class="mb-6">
            <label for="title" class="block text-gray-800 text-base font-semibold mb-2">Judul Promo:</label>
            <input type="text" id="title" name="title" value="<?= old('title', $promo['title']) ?>"
                ...
                placeholder="Misal: Diskon 20% Senin" required>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="5"
                class="appearance-none border border-gray-300 rounded-lg w-full py-3 px-4 text-gray-700 leading-tight
                       focus:outline-none focus:ring-4 focus:ring-accent-gold focus:border-transparent transition duration-300"
                placeholder="Detail promo"><?= old('description', $promo['description']) ?></textarea>
        </div>

        <div class="mb-8">
            <label for="current_image" class="block text-gray-800 text-base font-semibold mb-2">Gambar Saat Ini:</label>
            <?php if ($promo['image']): ?>
                <img src="<?= base_url($promo['image']) ?>" alt="Gambar Promo" class="w-40 h-40 object-cover rounded-lg mb-4 border-2 border-gray-300">
            <?php else: ?>
                <p class="text-gray-600 text-sm mb-4">Tidak ada gambar yang terunggah.</p>
            <?php endif; ?>
            <label for="image" class="block text-gray-800 text-base font-semibold mb-2 mt-4">Ganti Gambar Promo (Opsional):</label>
            ...
            <p class="text-xs text-gray-500 mt-2">Ukuran maksimal 1MB. Format: JPG, JPEG, PNG, WEBP. Kosongkan jika tidak ingin mengubah.</p>
        </div>

        <div class="flex flex-col sm:flex-row items-center justify-end gap-4">
            <a href="<?= site_url('admin/promos') ?>" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-3 px-6 rounded-full transition duration-300 border-2 border-gray-400 w-full sm:w-auto text-center">
                Batal
            </a>
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-full transition duration-300
                ...
                Perbarui Promo <span class="ml-2">&#x27A4;</span>
            </button>
        </div>
    </form>
</div>
